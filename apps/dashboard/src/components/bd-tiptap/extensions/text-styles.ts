...
const renderNumberToPx = (v: string): string => (/^\d+$/.test(String(v)) ? `${v}px` : v)
...
function createSubTextStyle(o: CreateStyleOptions): Extension<SubTextStylesOptions> {
...
function createStyle(o: CreateStyleOptions): Extension<SubTextStylesOptions> {
  const { name, styleName = name, cssName, renderValue = v => v } = o
  const fn = name.charAt(0).toUpperCase() + name.slice(1)
  return Extension.create<SubTextStylesOptions>({
    name,
